...
if ($path === '/api/admin/login' && $method === 'POST') {
    $d=body(); require_csrf($d); $u=(string)($d['username']??'');$p=(string)($d['password']??'');
    $eu=envv('ADMIN_USERNAME','admin');$ep=envv('ADMIN_PASSWORD','change-me-now');
    if(hash_equals($eu,$u)&&hash_equals($ep,$p)){session_regenerate_id(true);$_SESSION['admin']=true;$_SESSION['csrf']=bin2hex(random_bytes(24));json_out(['ok'=>true,'csrf'=>csrf()]);}
    json_out(['ok'=>false,'message'=>'Username/password salah.'],401);
}
if ($path === '/api/admin/logout' && $method === 'POST') { require_csrf(body()); $_SESSION=[]; session_destroy(); json_out(['ok'=>true]); }

if (str_starts_with($path,'/api/admin/')) {
    require_admin();
    if ($path==='/api/admin/me' && $method==='GET') json_out(['ok'=>true,'admin'=>true,'csrf'=>csrf()]);
    ...
    if ($path==='/api/admin/product' && $method==='POST') {
        $d=body();require_csrf($d);$id=(int)($d['id']??0);$name=clean((string)($d['name']??''));$price=max(0,(int)($d['price']??0));$quota=max(0,(int)($d['quota']??0));$desc=clean((string)($d['description']??''));$active=!empty($d['active']);
    ...
    if ($path==='/api/admin/product-delete' && $method==='POST') {$d=body();require_csrf($d);$id=(int)($d['id']??0);db()->prepare('DELETE FROM products WHERE id=?')->execute([$id]);json_out(['ok'=>true]);}
    ...
    if ($path==='/api/admin/order-status' && $method==='POST') {
        $d=body();require_csrf($d);$code=clean((string)($d['order_code']??''));$status=clean((string)($d['status']??''));
    ...
    if ($path==='/api/admin/settings' && $method==='GET') json_out(['ok'=>true,'settings'=>settings(),'csrf'=>csrf()]);
    if ($path==='/api/admin/settings' && $method==='POST') {
        $d=body();require_csrf($d);unset($d['csrf']);
